...
class Navigation extends Component {
	...
	renderScene(rt, navigator) {
        ...
        if (this.name){
            let _name = this.name;
            this.name = false;
            if (_.findIndex(_paths, {id: _name})>-1){
                return SAX.setter( _name, (this.intent||{}) )[0];
            }
        }
        else {
            if (rt && rt.id){
    ...
    distribute(name, intent, type){
        this.intent = intent;
        this.type = type;
        // _nav.jumpTo( _.find(_paths, {id: name}) )
        // _nav.push(_.find(_paths, {id: name}))
        ...
        switch (type) {
            case 'jumpTo':
                ...
                break;
            case 'replace':
                _nav.replace(_.find(_paths, {id: name}))
                break;
            case 'replacePrevious':
                _nav.replacePrevious(_.find(_paths, {id: name}))
                break;
            case 'popToRoute':
                _nav.popToRoute(_.find(_paths, {id: name}))
                break;
            case 'replacePreviousAndPop':
                _nav.replacePreviousAndPop(_.find(_paths, {id: name}))
                break;
            case 'resetTo':
                _nav.resetTo(_.find(_paths, {id: name}))
                break;
            default:
                _nav.push(_.find(_paths, {id: name}))
        }
	...
	render() {
		...
		return (
            <Navigator
                ref={view => this.navigator=view}
                initialRoute={_paths[ (() => _.findIndex(_paths, {id: this.name}))() ]}
                configureScene={this.configureScene.bind(this)}
                renderScene={this.renderScene.bind(this)}/>
		)
	}
