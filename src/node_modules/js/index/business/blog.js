...
function dealWithListItemData(data){
    ...
    if (data && data.length){
        data.map( (item, i)=>{
            ...
            let _dot = (
                <Image source={ images.leftTile } style={{width:30,height:30}} />
            )
            let _attach = (
                <Text>09:45</Text>
            )
            ...
            _data.push({ title: (
                <Item
                    ...
                    descript={item.title.substring(0,20)}
                    icon={_icon}
                    attach={_attach}
                    onPress={() => {
                        Router('web', {url: Config.domain+'?topic='+item._id})  // 路由到detail详情页，H5
                    }}
                />
                // <TouchableHighlight
                //     key={'titem'+i}
